...
const {
	fdkAxios,
} = require('@gofynd/fdk-client-javascript/sdk/common/AxiosHelper');
const {
	TEST_WEBHOOK_EVENT_NAME,
...
class WebhookRegistry {
	...
	async initialize(config, fdkConfig) {
		...
		if (
			...
			!emailRegex.test(config.notification_email)
		) {
		...
		config.subscribe_on_install =
			config.subscribe_on_install === undefined
				? true
				: config.subscribe_on_install;
		this._handlerMap = {};
		this._config = config;
		...
		for (let [eventName, handlerData] of Object.entries(
			this._config.event_map
		)) {
		...
		if (Object.keys(eventConfig.eventsNotFound).length) {
			...
			Object.keys(eventConfig.eventsNotFound).forEach((key) => {
				errors.push(
					`name: ${key}, version: ${eventConfig.eventsNotFound[key]}`
				);
			});
			throw new FdkInvalidWebhookConfig(
	...
	_validateEventsMap(handlerConfig) {
		...
		Object.keys(handlerConfig).forEach((key) => {
			if (
				!eventConfig.eventsMap.hasOwnProperty(
					`${key}/${handlerConfig[key].version}`
				)
			) {
	...
	_getEventIdMap(events) {
		return events.reduce((event_map, event) => {
			event_map[
				`${event.event_category}/${event.event_name}/${event.event_type}/${event.version}`
			] = event.id;
			return event_map;
		}, {});
	}
	...
	_associationCriteria(applicationIdList) {
		if (this._config.subscribed_saleschannel === 'specific') {
			return applicationIdList.length
				? ASSOCIATION_CRITERIA.SPECIFIC
				: ASSOCIATION_CRITERIA.EMPTY;
		}
	...
	_isConfigUpdated(subscriberConfig) {
		let updated = false;
		const configCriteria = this._associationCriteria(
		...
		if (this._webhookUrl !== subscriberConfig.webhook_url) {
			logger.debug(
				`Webhook url updated from ${subscriberConfig.webhook_url} to ${this._webhookUrl}`
			);
	...
	async syncEvents(platformClient, config = null, enableWebhooks) {
		...
		if (!subscriberConfig) {
			...
			logger.debug(
				`Webhook config on platform side for company id ${
					platformClient.config.companyId
				}: ${JSON.stringify(subscriberConfig)}`
			);
			const {
				id,
				name,
		...
		try {
			if (registerNew) {
				...
				if (this._fdkConfig.debug) {
					const event_map = Object.keys(eventConfig.eventsMap).reduce(
						(map, eventName) => {
					...
					subscriberConfig.event_id = subscriberConfig.event_id.map(
						(id) => event_map[id]
					);
					logger.debug(
						`Webhook config registered for company: ${
							platformClient.config.companyId
				...
				const eventDiff = [
					...subscriberConfig.event_id.filter(
						(eventId) => !existingEvents.includes(eventId)
					),
					...existingEvents.filter(
						(eventId) => !subscriberConfig.event_id.includes(eventId)
					),
				];
				...
				if (eventDiff.length || configUpdated) {
					...
					if (this._fdkConfig.debug) {
						...
						logger.debug(
							`Webhook config updated for company: ${
								platformClient.config.companyId
							}, config: ${JSON.stringify(subscriberConfig)}`
						);
					}
			...
			throw new FdkWebhookRegistrationError(
				`Failed to sync webhook events. Reason: ${ex.message}`
			);
		}
	}
	...
	async enableSalesChannelWebhook(platformClient, applicationId) {
		...
		if (this._config.subscribed_saleschannel !== 'specific') {
			throw new FdkWebhookRegistrationError(
				'`subscribed_saleschannel` is not set to `specific` in webhook config'
			);
		}
		try {
			let subscriberConfig = await this.getSubscriberConfig(platformClient);
			...
			throw new FdkWebhookRegistrationError(
				`Failed to add saleschannel webhook. Reason: ${ex.message}`
			);
		}
	}
	...
	async disableSalesChannelWebhook(platformClient, applicationId) {
		...
		try {
			...
			throw new FdkWebhookRegistrationError(
				`Failed to remove saleschannel webhook. Reason: ${ex.message}`
			);
		}
	}
	...
	verifySignature(req) {
		...
		const { body } = req;
		const calcSignature = hmacSHA256(
			JSON.stringify(body),
			this._fdkConfig.api_secret
		).toString();
	...
	async processWebhook(req) {
		...
		try {
			...
			const eventHandlerMap =
				this._handlerMap[categoryEventName] ||
				this._handlerMap[eventName] ||
				{};
			const extHandler = eventHandlerMap.handler;

			if (typeof extHandler === 'function') {
				logger.debug(
					`Webhook event received for company: ${
						req.body.company_id
					}, application: ${
				...
				throw new FdkWebhookHandlerNotFound(
	...
	async registerSubscriberConfig(platformClient, subscriberConfig) {
		...
		try {
			return await platformClient.webhook.registerSubscriberToEvent({
	...
	async updateSubscriberConfig(platformClient, subscriberConfig) {
		...
		try {
			return await platformClient.webhook.updateSubscriberConfig({
				body: subscriberConfig,
			});
		} catch (err) {
			if (
	...
	async getSubscriberConfig(platformClient) {
		...
		try {
			const subscriberConfig =
				await platformClient.webhook.getSubscribersByExtensionId({
					extensionId: this._fdkConfig.api_key,
				});
		...
		try {
			...
			Object.keys(handlerConfig).forEach((key) => {
				...
				if (eventDetails.length !== 3) {
					throw new FdkInvalidWebhookConfig(
						`Invalid webhook event map key. Invalid key: ${key}`
					);
				}
			...
			logger.debug(
				`Webhook events config received: ${logger.safeStringify(responseData)}`
			);
			return responseData;
		} catch (err) {
			if (
				...
				return await this._retryManager.retry(
					uniqueKey,
					this.getEventConfig.bind(this),
					handlerConfig
				);
			}

			throw new FdkInvalidWebhookConfig(
				`Error while fetching webhook events configuration, Reason: ${err.message}`
			);
		}
	}
}
